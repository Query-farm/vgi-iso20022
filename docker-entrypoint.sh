...
#
# Dispatch the single vgi-iso20022 image into one of its transports:
...
#
# The worker is stateless (message bytes are handed in per call), so there is no
...
case "${1:-http}" in
  http)
    ...
    exec iso20022-worker --http "$@"
    ;;
  tcp)
    shift 2>/dev/null || true
    exec iso20022-worker --tcp "0.0.0.0:${PORT_TCP:-8001}" "$@"
    ;;
  stdio)
    shift 2>/dev/null || true
    exec iso20022-worker "$@"
    ;;
  *)
    exec "$@"
    ;;
