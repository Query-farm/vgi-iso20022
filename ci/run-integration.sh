...
#
# Run this repo's sqllogictest suite (test/sql/*.test) against the vgi-iso20022
...
# is what LOCATION (VGI_ISO20022_WORKER) the .test files ATTACH:
#
#   subprocess  VGI_ISO20022_WORKER = the stdio worker command (DuckDB spawns it).
#   http        start `iso20022-worker --http` (auto port; advertises `PORT:<n>`
#               on stdout), VGI_ISO20022_WORKER = http://127.0.0.1:<port>.
#   unix        start `iso20022-worker --unix <sock>` (advertises `UNIX:<sock>`
#               on stdout), VGI_ISO20022_WORKER = unix://<sock>.
#
# Required environment:
#   HAYBARN_UNITTEST  path to the haybarn-unittest binary
#   WORKER_BIN        path to the compiled iso20022-worker binary.
# Optional:
...
WORKER_BIN="${WORKER_BIN:-$REPO/target/release/iso20022-worker}"
...
start_server_and_set_location() {
  local kind="$1"
  : "${WORKER_BIN:?path to the iso20022-worker binary (WORKER_BIN)}"
  ...
  case "$kind" in
    http)
      ...
      export VGI_ISO20022_WORKER="http://127.0.0.1:$port"
      echo "HTTP worker ready on 127.0.0.1:$port (pid $SERVER_PID)"
      ;;
    unix)
      SOCK_PATH="${VGI_ISO20022_SOCK:-/tmp/iso20022.$$.sock}"
      ...
      export VGI_ISO20022_WORKER="unix://$SOCK_PATH"
...
case "$TRANSPORT" in
  subprocess)
    export VGI_ISO20022_WORKER="${VGI_ISO20022_WORKER:-$WORKER_BIN}"
    ;;
  http)
    if [[ "${VGI_ISO20022_WORKER:-}" =~ ^https?:// ]]; then
      echo "Using pre-launched HTTP worker at $VGI_ISO20022_WORKER"
    else
...
: "${VGI_ISO20022_WORKER:?worker LOCATION (stdio command, http:// URL, or unix:// socket)}"

echo "Staging preprocessed tests into $STAGE ..."
mkdir -p "$STAGE/test/sql"
# Stage the fixture data alongside the tests so path-mode reads resolve.
mkdir -p "$STAGE/data"
cp -R "$REPO"/data/. "$STAGE/data/" 2>/dev/null || true
...
echo "Running suite (transport: $TRANSPORT, worker: $VGI_ISO20022_WORKER, pattern: $TEST_PATTERN) ..."
...
if grep -qiE "All tests were skipped|total skipped [1-9]" "$REPORT"; then
  ...
  echo "       is NOT a pass. Transport=$TRANSPORT worker=$VGI_ISO20022_WORKER." >&2
  exit 1
fi
