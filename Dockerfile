...
#
# Single image that serves every network transport of the `iso20022` VGI worker:
...
#
# Like vgi-units this worker is STATELESS: the SWIFT MT / ISO 20022 MX parse
# engine is compiled into the binary and reads message bytes handed to it per
# call — there is no writable state, no model registry, and no
...
# runtime ships. `git` is preinstalled in the rust image.
...
RUN --mount=type=cache,target=/usr/local/cargo/registry \
    ...
    cargo build --release --locked --bin iso20022-worker \
    && cp target/release/iso20022-worker /iso20022-worker
...
ARG SOURCE_URL=https://github.com/Query-farm/vgi-iso20022
...
LABEL org.opencontainers.image.title="vgi-iso20022" \
      org.opencontainers.image.description="Parse SWIFT MT & ISO 20022 MX payment messages into typed DuckDB rows as a VGI worker (stdio + HTTP + TCP)" \
...
ENV PORT=8000 \
    ...
    VGI_ISO20022_GIT_COMMIT=${GIT_COMMIT}
...
COPY --from=build --chmod=0755 /iso20022-worker /usr/local/bin/iso20022-worker
